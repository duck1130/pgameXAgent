...
**Survival Realm** 是一款使用 Pygame 開發的 **2D 生存 RPG 遊戲**，採用完全重構的模組化架構 (v3.1.0)。玩家需要收集資源、製作工具、在晝夜循環中對抗怪物並建造建築物。程式碼遵循嚴格的配置驅動設計，具有集中式狀態管理和完整的型別註解。

> 「這麼簡單都不會！本大爺只好勉為其難地幫你！」 - 硬漢貓咪開發團隊 (ˋ・ω・ˊ)
...
- `ITEM_RECIPES`, `WORLD_OBJECTS`, `TOOL_EFFICIENCY` 定義所有遊戲機制
